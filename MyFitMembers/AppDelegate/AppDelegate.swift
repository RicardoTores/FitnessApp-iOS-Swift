...
//        Instabug.startWithToken("9cec6583e70b03711b062772715cb5eb", invocationEvent: .Shake)
        guard let pushNotification = launchOptions?[UIApplicationLaunchOptionsRemoteNotificationKey] else {
            return true }
        UserDataSingleton.sharedInstance.pushDict = pushNotification as? NSDictionary
        NSUserDefaults.standardUserDefaults().setObject(pushNotification, forKey: "Push")
        return true
    }
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("MyFitMembers", withExtension: "momd")!
    ...
    func application(application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceTokn: NSData) {
        var tokenString = ""
        let tokenChars = UnsafePointer<CChar>(deviceTokn.bytes)
        for i in 0..<deviceTokn.length {
    ...
    func logOut(){
        ...
        NSUserDefaults.standardUserDefaults().setValue(nil, forKey: "DeviceTokenUpdated")
