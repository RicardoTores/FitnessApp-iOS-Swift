...
public class KDCircularProgress: UIView {
    ...
    private struct UtilityFunctions {
        ...
        static func Mod(value: Float, range: Float, minMax: (Float, Float)) -> Float {
            let (min, max) = minMax
    ...
    @IBInspectable public var angle: Float = 0.0 {
        didSet {
            if self.isAnimating() {
                self.pauseAnimation()
            }
    ...
    public func animateFromAngle(fromAngle: Float, toAngle: Float, duration: NSTimeInterval, relativeDuration: Bool = true, completion: ((Bool) -> Void)?) {
        if isAnimating() {
            pauseAnimation()
        }
    ...
    public func animateToAngle(toAngle: Float, duration: NSTimeInterval, relativeDuration: Bool = true, completion: ((Bool) -> Void)?) {
        if isAnimating() {
            pauseAnimation()
        }
    ...
    private class KDCircularProgressViewLayer: CALayer {
        ...
        private struct GlowConstants {
            ...
            static func glowAmountForAngle(angle: Float, glowAmount: CGFloat, glowMode: KDCircularProgressGlowMode, size: CGFloat) -> CGFloat {
                switch glowMode {
                case .Forward:
